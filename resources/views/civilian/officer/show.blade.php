...
@section('content')
    <div class="card">
        <div class="lg:flex lg:justify-between py-2 border-b-2">
            <h2 class="text-2xl text-white">{{ $officer->name }} (SSN: {{ $officer->s_n_n }})</h2>
            <div class="flex space-x-2">
                <a class="secondary-button-sm" href="{{ route('civilian.officers.index') }}">
                ...
                <a class="edit-button-sm" href="{{ route('civilian.officers.edit', $officer->id) }}">
                    <x-edit-button></x-edit-button>
                </a>
                <form action="{{ route('civilian.officers.destroy', $officer->id) }}" method="POST"
                    onsubmit="return confirm('Are you sure you wish to delete this officer? This can\'t be undone!');">
                    @csrf
                    @method('DELETE')
        ...
        <div class="flex flex-wrap pt-4 -mx-4">
            <div class="w-full px-4 md:w-1/2">
                @if (!is_null($officer->picture))
                    <img class="block h-64 rounded-md" src="{{ $officer->picture }}">
                @else
            ...
            <div class="w-full px-4 text-white md:w-1/2">
                <p><span class="text-gray-300">Full Name:</span> {{ $officer->name }}</p>
                <p><span class="text-gray-300">Social Security Number:</span> {{ $officer->id }}</p>
                <p><span class="text-gray-300">Date of Birth:</span> {{ $officer->date_of_birth->format('m/d/Y') }}
                    (Age: {{ $officer->age }})
                </p>
                <p><span class="text-gray-300">Gender:</span> {{ $officer->gender }}</p>
                <p><span class="text-gray-300">Race:</span> {{ $officer->race }}</p>
                <p><span class="text-gray-300">Weight:</span> {{ $officer->weight }}lb
                    ({{ round($officer->weight / 2.205) }}kg)</p>
                <p><span class="text-gray-300">Height:</span> {{ floor($officer->height / 12) }}'
                    {{ $officer->height % 12 }}"
    ...
    <div class="card">
        <div class="flex justify-between py-2 border-b-2">
            <h2 class="text-2xl text-white">Department Information</h2>
            <div class="flex">
        ...
        <div class="flex flex-wrap -mx-4">
            <div class="w-full px-4 flex justify-between">
                <div class="text-white">
                    ...
                    <p>Rank: {{ $officer->user_department->rank }}</p>
                </div>
                <div class="p-3">
                    <img alt="Department Logo" class="h-32" src="{{ $officer->user_department->department->logo }}">
                </div>
            </div>
        </div>
    </div>
