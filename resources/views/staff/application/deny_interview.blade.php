...
@section('content')
    <header class="w-full my-3">
        <h1 class="text-2xl font-bold text-white">Deny Interview</h1>
    ...
    <div class="card">
        ...
        <form action="{{ route('staff.application.deny_interview.store', $application->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="w-full">
                <label for="denied_reason" class="block mt-3 text-black-500">Why are you denying this
                    application?</label>
                <textarea type="text" name="denied_reason" class="textarea-input">{{ old('denied_reason') }}</textarea>
                <x-input-error :messages="$errors->get('denied_reason')" class="mt-2" />
            </div>

            <div class="mt-4">
                <button class="w-1/3 mr-5 bg-yellow-600 button-md hover:bg-yellow-500">Deny Interview</button>
